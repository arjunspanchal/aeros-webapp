...
async function translateFormulaToFilter(formula, cfg) {
  ...
  // LOWER({Field})='value'
  //
  // Original implementation translated to `eq.<lowercased>`, which only matched
  // when the underlying column was citext (or already stored lowercase). For
  // plain `text` columns (e.g. clients.name) that meant the case-insensitive
  // dedup checks in route handlers silently failed and produced duplicate rows
  // — see signup/route.js's `upsertClientForCompany`. Switching to PostgREST
  // `ilike` makes the comparison genuinely case-insensitive on either column
  // type. `*` is PostgREST's wildcard in ilike values, so escape literal `*`s
  // to keep this an exact match.
  m = f.match(/^LOWER\(\{([^}]+)\}\)='((?:[^']|\\')*)'$/);
  if (m) {
    const col = pgColForAirtableField(cfg, m[1]);
...
export async function airtableList(table, { filterByFormula, sort, maxRecords, pageSize } = {}) {
  ...
  // Shapes with per-row joins (Products photos, Jobs client/manager/LR files)
  // declare buildBatchContext to prefetch those once for the whole result set,
  // collapsing an N+1 into a handful of queries. toAirtable reads from ctx when
  // present and falls back to its per-row query otherwise (airtableGet path).
  const ctx = typeof cfg.buildBatchContext === "function"
    ? await cfg.buildBatchContext(rows)
    : undefined;
  return Promise.all(rows.map((r) => Promise.resolve(cfg.toAirtable(r, ctx))));
}
