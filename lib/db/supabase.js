 ...
 * A filter value may be an array to apply multiple conditions to the same
 * column (PostgREST AND-combines repeated params), e.g.
 *   filter: { movement_date: ["gte.2026-06-01", "lte.2026-06-30"] }
 */
export async function dbSelect(table, opts = {}) {
  ...
  for (const [col, expr] of Object.entries(filter)) {
    if (Array.isArray(expr)) {
      for (const e of expr) url.searchParams.append(col, e);
    } else {
      url.searchParams.append(col, expr);
    }
  }
