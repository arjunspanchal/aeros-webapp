...
export const SHAPES = {
  ...
  Jobs: {
    ...
    toPg: async (f) => {
      ...
      if ("Brand" in f) out.brand = str(f.Brand);
      // Master SKU drives master_sku, master_product_id (FK) AND
      // master_product_name (snapshot). Resolving them as a single atomic
      // unit closes two audit findings:
      //   M1: when the SKU doesn't resolve to a master_products row, we
      //       throw — previously master_product_id silently went NULL,
      //       leaving an unjoinable job that wouldn't push to the FG
      //       ledger and that downstream reports treated as "unmapped".
      //   M2: when the SKU resolves, we override master_product_name
      //       from the canonical row, ignoring whatever the client sent.
      //       Previously masterSku and masterProductName were set
      //       independently — if a client sent the right SKU with a
      //       stale or wrong name, both would land in the DB out of
      //       sync, and the FG ledger / customer-facing UI would split
      //       on which one they read.
      const hasSku  = "Master SKU" in f;
      const hasName = "Master Product Name" in f;
      if (hasSku) {
        ...
        if (masterSku) {
          const rows = await dbSelect("master_products", {
            ...
            filter: { sku: `eq.${masterSku}` },
            limit: 1,
          });
          const hit = rows[0];
          if (!hit) {
            const err = new Error(
              `Master SKU "${masterSku}" not found in the master catalogue. Pick a product from the dropdown or have an admin add the SKU first.`,
            );
            err.code = "unknown_master_sku";
            throw err;
          }
          out.master_product_id = hit.id;
          // M2: canonical name wins. Caller's "Master Product Name" (if any)
          // is ignored when the SKU resolves — that's the snapshot.
          out.master_product_name = hit.product_name || null;
        } else {
          // Caller explicitly cleared the SKU. Wipe both FK + snapshot.
          out.master_product_id = null;
          out.master_product_name = null;
        }
      } else if (hasName) {
        // SKU not in the patch but name is — rare path (e.g. someone
        // editing only the display name). Honour it; M2 only applies
        // when the SKU is the driver.
        out.master_product_name = str(f["Master Product Name"]);
      }
  ...
  Products: {
    ...
    selectColumns: "*",
    toAirtable: async (row) => {
      const photos = await buildPublicAttachmentArrayFromJoin(
