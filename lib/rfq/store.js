 ...
 *   - clientEmail  — limit to one customer (used for customer view)
 *   - clientId     — limit to one client by uuid
 *   - search       — case-insensitive match on either RFQ number or product
 */
export async function listRfqQuotes({ clientEmail, clientId, search } = {}) {
  const filter = {};
...
/**
 * Update an existing quote. Three workflows compose into one helper:
 *
 *   1. Pure metadata update — pass any subset of editable fields, no
 *      file payload. Existing PDF is left untouched.
 *   2. Replace PDF — include `filename` + `fileBase64` (and optionally
 *      `contentType` / `bytes`). The new file uploads to a fresh path,
 *      the row is patched, and the old blob is deleted only after the
 *      metadata update succeeds (so a partial failure doesn't strand
 *      the row pointing at deleted storage).
 *   3. Remove PDF — pass `clearPdf: true`. Row metadata stays; the
 *      filename / storage_path / content_type / bytes columns clear
 *      and the existing blob is deleted from storage.
 *
 * Workflows 2 + 3 are mutually exclusive — when `clearPdf` is true any
 * file payload in the same call is ignored. Caller is responsible for
 * admin auth.
 */
export async function updateRfqQuote(id, input = {}) {
  ...
  // Storage operations are deferred until after the metadata update so
  // a partial failure rolls back cleanly.
  let pathToDeleteAfter = null; // old blob (replace or clear case)
  let uploadedPath = null;       // new blob (replace case) — for cleanup on metadata error
  ...
  if (input.clearPdf) {
    ...
    patch.bytes = null;
    pathToDeleteAfter = existing.storage_path;
  } else if (input.fileBase64 || input.bytes) {
    if (!input.filename) throw new Error("filename is required when uploading a PDF");
  ...
  try {
    ...
    // Metadata failed — clean up the new blob if we already uploaded one
    // so we don't leak storage.
    if (uploadedPath) {
      try { await deleteFromBucket(BUCKET, uploadedPath); } catch {}
    }
    throw err;
  }

  // Metadata update succeeded — safe to drop the old blob (replace + clear).
  if (pathToDeleteAfter && pathToDeleteAfter !== uploadedPath) {
    try { await deleteFromBucket(BUCKET, pathToDeleteAfter); } catch {}
  }
