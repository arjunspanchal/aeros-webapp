...
/**
 * Patch the metadata fields on an existing RFQ row. The file itself is
 * immutable here — replacing the PDF means deleting + re-uploading via
 * createRfqQuote (the storage path encodes a timestamp so re-uploads are
 * always new objects). Whitelist below stops a stray frontend payload
 * from rewriting fields it shouldn't (storage_path, content_type, etc.).
 *
...
export async function updateRfqQuote(id, input) {
  ...
  if (input.aerosRfqNumber !== undefined) {
    ...
    if (!v) throw new Error("aerosRfqNumber cannot be blank");
    patch.aeros_rfq_number = v;
  }
  if (input.customerRfqNumber !== undefined) {
    patch.customer_rfq_number = input.customerRfqNumber ? String(input.customerRfqNumber).trim() : null;
  }
  if (input.clientId !== undefined) patch.client_id = input.clientId || null;
  if (input.clientEmail !== undefined) {
    patch.client_email = input.clientEmail ? String(input.clientEmail).toLowerCase() : null;
  }
  if (input.brand !== undefined) patch.brand = input.brand ? String(input.brand).trim() : null;
  if (input.productName !== undefined) patch.product_name = input.productName ? String(input.productName).trim() : null;
  if (input.notes !== undefined) patch.notes = input.notes ? String(input.notes) : null;

  if (Object.keys(patch).length === 0) {
    // Nothing to patch — return the current row so callers can refresh
    // their state without a no-op DB write.
    return getRfqQuote(id);
  }

  patch.updated_at = new Date().toISOString();

  const { dbUpdate } = await import("@/lib/db/supabase");
  await dbUpdate(TABLE, "id", id, patch);
  return getRfqQuote(id);
}
