...
// PET cups come in a handful of top diameters — Ø92/Ø98 (India, Oracle) and
// Ø78/Ø89 (China) — so only round lids of those diameters actually fit. Square
// lids and other off-size rounds (62/75/80/85/87/90/148mm) belong to other cup
// lines, and the 72/73mm lids are for the ICT range, not PET. We filter lids to
// these diameters before listing.
const PET_LID_DIAMETERS = new Set([78, 89, 92, 98]);
...
export async function fetchPetCupsAndLids() {
  ...
  const [cups, lids] = await Promise.all([
    dbSelect("master_products", {
      select: "id,sku,product_name,category,sub_category,size_volume,material,colour,units_per_case,country_of_origin",
