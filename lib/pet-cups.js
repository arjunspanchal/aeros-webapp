...
export async function fetchPetCupsAndLids() {
  ...
  for (const item of products) {
    ...
    const row = {
      sku: item.sku,
      name: isLid ? cleanLidName(item.product_name) : cleanName(item.product_name),
      isLid,
...
// Lid display name: keep the style words ("Sipper Lid", "High Dome Lid",
// "Flat Lid (Straw Cut)") — they distinguish variants within a section — and
// strip only the size prefix, "PET" and the "- TF" process suffix. Unlike cups,
// a fully stripped lid name turns cryptic ("—", "N-Straight"), so the type
// words stay.
