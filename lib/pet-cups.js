...
// them:
//   • plain    — unprinted item, plain offering on the base SKU.
//   • printed  — custom print, taken from the paired "-CUST" twin SKU's
//                `generic_printed` offering (separate SKU per the
//                plain-vs-printed convention, like paper cups/bags). Lids are
...
// Product photos live in public/pet-cups/{cups,lids}/<SKU>.<ext> (drop-in by
// SKU, same mechanism as the lid gallery). Scanned server-side so each row can
// carry its image URL.
function imageMap(dir) {
  const map = new Map();
  try {
    for (const f of fs.readdirSync(path.join(process.cwd(), "public", "pet-cups", dir))) {
      const m = f.match(/^(.+)\.(jpe?g|png|webp)$/i);
      if (m) map.set(m[1], `/pet-cups/${dir}/${f}`);
    }
  } catch {
    // folder may not exist in some environments — rows just render without thumbs
  }
  return map;
}
...
export async function fetchPetCupsAndLids() {
  ...
  // Index by sku so a base cup can find its "-CUST" printed twin (separate SKU
  // per the plain-vs-printed convention; twins fold into the base row below).
  const idBySku = new Map();
  for (const p of products) idBySku.set(p.sku, p.id);
  ...
  const pricing = await dbSelect("master_product_pricing", {
    ...
    filter: {
      ...
      offering_type: "in.(plain,generic_printed,custom_branded)",
    },
  });
  ...
  const cupImgs = imageMap("cups");
  const lidImgs = imageMap("lids");
  ...
  for (const item of products) {
    if (/-CUST$/i.test(item.sku)) continue; // printed twins fold into their base row
    ...
    // base row itself if it still carries a printed offering).
    const twinId = idBySku.get(`${item.sku}-CUST`);
    const printed = shapeOffering([
      ...(printedByProduct.get(item.id) || []),
    ...
    const row = {
      sku: item.sku,
      name: isLid ? cleanLidName(item.product_name) : cupName(item.product_name),
      img: (isLid ? lidImgs : cupImgs).get(item.sku) || null,
      isLid,
      volume: isLid ? null : volumeOf(item.size_volume),
      oz: isLid ? null : ozOf(item.size_volume),
      // Bottom profile (cups only): "U-Bottom" | "F-Bottom" — drives the
      // bottom filter chips.
      bottom: isLid ? null : (/U[-\s]?Shape/i.test(item.product_name || "") ? "U-Bottom" : "F-Bottom"),
...
// Cup display name: lead with the bottom profile — "U-Bottom" for U-shape
// cups, "F-Bottom" for flat ones — so the Item column always says which
// profile the cup is, followed by any variant tag (e.g. "F-Bottom · 9806").
function cupName(name) {
  const isU = /U[-\s]?Shape/i.test(name || "");
  const rest = cleanName(name)
    .replace(/U[-\s]?Shape/gi, "")
    .replace(/^[\s,;·—-]+|[\s,;·—-]+$/g, "")
    .trim();
  const bottom = isU ? "U-Bottom" : "F-Bottom";
  return rest ? `${bottom} · ${rest}` : bottom;
}

// Lid display name: just the style word — "Sipper", "Dome", "High Dome",
// "Flat" — plus any variant qualifier ("Flat (Straw Cut)", "Sipper (N-Type)").
// The size prefix, "PET", "Lid" and the "- TF" process suffix are stripped;
// unlike cups, the style word itself stays so a row is never cryptic or blank.
function cleanLidName(name) {
  if (!name) return "";
  return name
...
function cleanName(name) {
  ...
  let s = name
    ...
    .replace(/\bCup\b/gi, "")
    .replace(/\bLid\b/gi, "")
    // TD/BD markers ride along with the stripped sizes ("(98mm TD)" → "( TD)")
    // and mean nothing alone — drop them too.
    .replace(/\bTD\b/g, "")
    .replace(/\bBD\b/g, "");
  s = s
    .replace(/\s{2,}/g, " ")
