...
// + the plain pricing rows from `master_product_pricing`, joins them in JS, and
// shapes a render-ready list grouped by material (Paper / Rice).
//
// Straws are a plain-only line — no printing, no quantity ladder. Each item
// carries a single MOQ slab with two pricing bases:
//   • priceInr — EXW India (FCL / export) rate, already live on the sheet.
//   • ddpInr   — India delivered duty-paid rate from `india_landed_inr`. Null
//                where not yet costed → shown "on request" under the DDP basis.
// Items with no priced slab at all (e.g. the rice range, not yet costed) fall
// through to "on request" under both bases. Rates are INR only; USD is an
// indicative display conversion.
...
export async function fetchStraws() {
  ...
  // Collect priced slabs per product. priceInr is the EXW India (FCL) rate;
  // ddpInr is the India delivered rate (null where not yet costed).
