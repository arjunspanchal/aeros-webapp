...
// Stage transition policy (audit H4). Until now, anyone who could
// updateStage could write any stage at any time — including jumping
// "Dispatched" back to "RM Pending", or skipping "Delivered" off a
// "Packing" job. Customers see whatever stage was last written, and
// the FG-ledger logic (push-to-warehouse) implicitly assumes a
// monotonic flow. The rules below codify that flow at the API layer.
//
// Allowed transitions:
//   • From == To                    — always (no-op resaves).
//   • Customer Dispatched→Delivered — always, handled inline by the
//                                     PATCH route via isCustomerDeliver.
//   • Admin / Factory Manager       — any → any (including backward
//                                     for corrections, e.g. RM came
//                                     back to be recut).
//   • Factory Executive             — forward-only.
//   • Account Manager               — forward-only.
//   • Customer                      — Dispatched→Delivered only (the
//                                     isCustomerDeliver path above).
//
// "Forward" means strictly later in STAGES (STAGE_INDEX[to] > STAGE_INDEX[from]).
// Skipping intermediate stages forward is fine — production lines do that
// regularly (e.g. RM Pending → Packing if the printer was idle).
export function isAllowedStageTransition({ from, to, role, isCustomerDeliver }) {
  if (from === to) return true;
  if (isCustomerDeliver) return true;
  ...
  if (role === ROLES.FACTORY_EXECUTIVE || role === ROLES.ACCOUNT_MANAGER) {
