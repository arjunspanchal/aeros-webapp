...
export async function getRunFeeds(runId) {
  const rows = await dbSelect("production_run_feeds", {
    select: "id,role,role_label,rm_roll_id,sku_id,sku_snapshot,qty_pcs,consumed_kg",
...
export async function startRun({
  ...
  // Every roll slot must be filled; sku slots must carry a sku.
  ...
  const feedRows = feeds.map((f) => ({
    ...
    sku_snapshot: f.kind === "sku" ? (f.skuSnapshot || "") : null,
