...
//       access = "customer" → a customer user linked to one of this job's clients
...
export async function resolveJobAccess(session, jobId) {
  ...
  if (requireRole(session, "factoryos", "vendor")) {
    ...
    if (vendorOwnsJob(job, session.factoryosVendorId, vendor?.name)) {
      return { job, access: "vendor", vendor };
    }
  }

  if (requireRole(session, "factoryos", ROLES.CUSTOMER)) {
    const myClients = new Set(session.factoryosClientIds || []);
    if (job.clientIds.some((c) => myClients.has(c))) {
      return { job, access: "customer", vendor: null };
    }
  }
