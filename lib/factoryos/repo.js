...
export async function listJobsForSession(session) {
  ...
  if (session.role === ROLES.VENDOR) {
    // Printing vendors see only jobs assigned to their vendor record. Primary
    // match is the robust printing_vendor_id FK; a case-insensitive name match
    // on the legacy text snapshot is a fallback for any job not yet linked.
    if (!session.vendorId && !session.vendorName) return [];
    const vid = session.vendorId || null;
    const vname = (session.vendorName || "").trim().toLowerCase();
    return jobs.filter(
      (j) =>
        (vid && j.printingVendorId === vid) ||
        (vname && (j.printingVendor || "").trim().toLowerCase() === vname),
    );
  }
  return [];
}
...
export async function postJobMessage({
  ...
  const row = await dbInsert(
    "job_messages",
    {
      ...
      author_role: authorRole === "vendor" ? "vendor" : "team",
      kind,
      ...attach,
    },
    { returning: "representation" },
...
// surfaced as unread. side = 'vendor' | 'team'.
export async function markThreadRead(jobId, side) {
  const pgId = await jobPgId(jobId);
  if (!pgId) return;
  const col = side === "vendor" ? "vendor_last_read_at" : "team_last_read_at";
...
export async function setVendorStatus(jobId, status, { dispatchDate } = {}) {
  ...
  if (!valid.has(status)) throw new Error("Invalid vendor status");
  const patch = {
