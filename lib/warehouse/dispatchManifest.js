...
const LINE_SELECT =
  "id,dispatch_id,sr_no,master_product_id,item_id,sku,description,box_count," +
...
// Replace the whole manifest in one shot — the form edits the list client-side
// and saves it entire, mirroring how sample-dispatch items are written. Simpler
// than diffing, and a manifest is small.
export async function replaceManifestLines(dispatchId, lines = []) {
  const clean = [];
  lines.forEach((l, idx) => {
  ...
  if (clean.length) await dbInsert("vehicle_dispatch_lines", clean);
  return listManifestLines(dispatchId);
}
