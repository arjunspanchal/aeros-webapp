...
// The invoice team logs each outbound vehicle: the customer (from the clients
// master), the transporter (a shared-directory vendor of type 'Transport'),
// vehicle size, box count, gross weight, the lane, and the lump-sum freight
// quote. ₹/box and ₹/kg are derived here so they can never drift from the
// stored freight on an edit.
//
// Invoices and their e-way bills live one-per-row in vehicle_dispatch_invoices
// (see dispatchManifest.js) — a vehicle routinely carries several, to more than
// one consignee. `customer_name` here is the account that owns the trip, not
// necessarily the only consignee on it.
//
...
import { listInvoicesForDispatches } from "./dispatchManifest.js";
...
// Vehicle bodies + the CBM→vehicle recommender live in vehicleCapacity.js —
// a pure module with no DB import, so the manifest calculator can run the same
// maths in the browser. Re-exported here so existing server call-sites keep
// importing VEHICLE_SIZES from this module.
export {
  VEHICLE_SPECS,
  VEHICLE_SIZES,
  VEHICLE_FILL_FACTOR,
...
const SELECT =
  "id,dispatch_no,dispatch_date,client_id,customer_name," +
  "account_manager_user_id,account_manager_name," +
  ...
  "box_count,total_weight_kg," +
  "from_city,from_place_id,from_lat,from_lng," +
  "to_city,to_place_id,to_lat,to_lng," +
  "approx_kms,freight_lumpsum_inr," +
...
export async function listVehicleDispatches({ limit = 500 } = {}) {
  ...
  const dispatches = rows.map(normalize);

  // Attach each vehicle's invoices in one follow-up rather than a query per
  // row, so the queue can show "INV-1 (+2)" and search across all of them.
  const byDispatch = await listInvoicesForDispatches(dispatches.map((d) => d.id)).catch(
    () => new Map(),
  );
  return dispatches.map((d) => ({ ...d, invoices: byDispatch.get(d.id) || [] }));
}
...
function buildPayload(payload) {
  return {
    ...
    account_manager_user_id: payload.account_manager_user_id || null,
    account_manager_name:    payload.account_manager_name?.trim() || null,
    ...
    from_place_id:    payload.from_place_id?.trim() || null,
    from_lat:         numOrNull(payload.from_lat),
    from_lng:         numOrNull(payload.from_lng),
    to_city:          payload.to_city?.trim() || null,
    to_place_id:      payload.to_place_id?.trim() || null,
    to_lat:           numOrNull(payload.to_lat),
    to_lng:           numOrNull(payload.to_lng),
...
// Account managers for the "who owns this consignment" dropdown — the same
// users directory FactoryOS assigns jobs from, so the two modules can never
// disagree about who the AMs are. Read straight from Supabase (not the legacy
// Airtable listUsers) and name-sorted.
export async function listAccountManagers() {
  const rows = await dbSelect("users", {
    select: "id,name,email,factoryos_role,active",
    filter: { factoryos_role: `eq.${ROLES.ACCOUNT_MANAGER}` },
    order: "name.asc",
    range: "0-999",
  });
  return rows
