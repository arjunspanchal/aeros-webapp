...
// Recently-used From/To locations across past dispatches, so the pickers can
// offer one-tap reuse before the user types. Pulls the last ~80 dispatches,
// flattens both endpoints, and dedupes by place_id keeping the most recent
// label/coords. Only resolved (place_id-bearing) locations are returned —
// those carry coordinates, so picking one still drives the kms auto-fill.
export async function listRecentLocations(limit = 8) {
  const rows = await dbSelect("vehicle_dispatches", {
    select: "from_city,from_place_id,from_lat,from_lng,to_city,to_place_id,to_lat,to_lng,created_at",
    filter: { deleted_at: "is.null" },
  ...
  const seen = new Map();
  const consider = (city, placeId, lat, lng) => {
    if (!placeId || !city) return;
    if (seen.has(placeId)) return;        // rows are newest-first, so first wins
    seen.set(placeId, {
      place_id: placeId,
      label: city,
  ...
  for (const r of rows) {
    consider(r.from_city, r.from_place_id, r.from_lat, r.from_lng);
    consider(r.to_city, r.to_place_id, r.to_lat, r.to_lng);
  }
