...
// Per-item on-hand, keyed by item_id, with the location breakdown. Powers the
// movement form: the item picker shows on-hand, and the outward "from location"
// dropdown is restricted to locations that actually hold the item.
export async function listStockPositions() {
  const rows = await dbSelect("inventory_stock_position", {
    select: "item_id,total_qty,by_location",
    limit: 10000,
  });
  return rows.map((r) => ({
    item_id: r.item_id,
