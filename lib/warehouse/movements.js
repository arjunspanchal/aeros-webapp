...
export async function listMovements({
  ...
  // Date range — PostgREST AND-combines repeated params for the same column,
  // so an array applies both bounds in a single query. (Previously the two
  // bounds were jammed into one filter string, which Postgres rejected with
  // "invalid input syntax for type date".)
  if (fromDate && toDate) {
    filter.movement_date = [`gte.${fromDate}`, `lte.${toDate}`];
  } else if (fromDate) {
    filter.movement_date = `gte.${fromDate}`;
  } else if (toDate) {
    filter.movement_date = `lte.${toDate}`;
  }
