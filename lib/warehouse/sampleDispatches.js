...
const DETAIL_SELECT =
  ...
  "sample_dispatch_items(id,sr_no,order_id,description,quantity,price,gst_pct,master_product_id,sample_kit_id)";
...
// Status / courier / AWB updates from the detail page action bar. Kept
// distinct from updateDispatch so the cheap status flip doesn't have to
// build and post a full edit payload.
...
// Full edit from /warehouse/sample-dispatch/[id]/edit — replaces header
// fields and the entire item list. Preserves order_id on lines that
// already have one (so the printed CBX######## tracking IDs survive an
// edit) and generates fresh ones for new lines.
export async function updateDispatch(id, payload) {
  ...
  await dbUpdate("sample_dispatches", "id", id, headerPatch);

  await dbDelete("sample_dispatch_items", "dispatch_id", id);
  const rows = items.map((ln, idx) => ({
    dispatch_id: id,
    sr_no:       idx + 1,
    order_id:    (ln.order_id || generateOrderId()).trim(),
