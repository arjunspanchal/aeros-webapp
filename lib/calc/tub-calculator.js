...
export const TUB_SIZE_OPTS = ["12oz_115", "14oz_115"];
export const TUB_SIZE_LABELS = {
  "12oz_115": "12 oz · 115 mm",
  "14oz_115": "14 oz · 115 mm",
};

// Sidewall blank [length_mm, width_mm], fans per sheet, bottom roll dia,
// and finished tub td/bd/h. Approximate defaults — refine against Aeros's
// tub spec sheet.
export const TUB_DIMS = {
  "12oz_115": { sw: [370, 90],  fans: 5, bottomRollDia: 95, td: 115, bd: 95, h: 75 },
  "14oz_115": { sw: [370, 105], fans: 5, bottomRollDia: 95, td: 115, bd: 95, h: 90 },
};
...
// Bottom paper stays locked at 230 GSM + 2PE coating (same as cups).
...
export function calculateTub(f) {
  ...
  // Bottom disc — tub-specific roll diameter, still 230 GSM + 2PE locked
  const btR = (dims.bottomRollDia / 2) / 1000;
  const btTotalRate = (parseFloat(f.btRate) || 0) + effectiveCoatingRate(LOCKED_BT_COATING, f.btCoatingRate);
  const btCost = Math.PI * btR * btR * (LOCKED_BT_GSM / 1000) * btTotalRate;
  const btWeightG = Math.PI * btR * btR * (LOCKED_BT_GSM / 1000) * 1000 * WEIGHT_CORRECTION;
