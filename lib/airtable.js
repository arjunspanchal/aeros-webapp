...
function buildWhatsAppUrl(item) {
  const msg = `Hi, I'm interested in this item from the Aeros Warehouse — Item: ${item.itemName} — Brand: ${item.brand} — Category: ${item.category} — Stock available: ${item.stockQuantity ?? "TBC"} ${item.unit} — Could you share more details and pricing?`;
  return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`;
}

function buildEmailUrl(item) {
  const subject = `Inquiry: ${item.itemName}`;
  const body = `Hi, I'm interested in this item from your warehouse stock — Item: ${item.itemName} — Brand: ${item.brand} — Category: ${item.category} — Stock available: ${item.stockQuantity ?? "TBC"} ${item.unit} — Please share more details and pricing. Thanks`;
 ...
 */
export async function fetchInventory({ internal = false } = {}) {
  const items = await dbSelect("clearance_items", {
    select: "id,airtable_id,item_name,brand,category,stock_quantity,unit,case_pack,price,price_unit,description,specifications,status,is_internal,clearance_item_photos(storage_path,sort_order)",
    order: "item_name.asc",
    range: "0-9999",
  });

  const normalized = items.map(normalizeRow).filter(Boolean);
  // Default = public list (branded dead stock and other internal-only
  // rows hidden). Pass {internal:true} to get the staff-only list of
  // is_internal rows for the team-facing Branded Dead Stock tab.
  const filtered = normalized.filter((item) =>
    internal ? item.isInternal : !item.isInternal,
  );
...
function normalizeRow(row) {
  ...
  const item = {
    ...
    // Optional override for the price denominator (e.g. RM stocked in
    // sheets but priced per kg). Falls back to `unit` in the renderer.
