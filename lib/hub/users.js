...
export async function resolveEntitlements(email) {
  ...
  const explicitCalc = c && CALC_ROLES.has(c) ? c : null;
  // Client calculator is visible for every authenticated user. Admin calc
  // (full breakdown, no margin) stays explicit via calculator_role=admin.
  const calc = explicitCalc || (orders ? "client" : null);
  if (!calc && !orders) return null;
  // Rate cards: admins and customer managers (account_manager) edit;
  // everyone else with platform access reads their own.
  const rateCards = orders === "account_manager" ? "admin" : (explicitCalc || "client");
