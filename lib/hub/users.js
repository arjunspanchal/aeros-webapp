...
export async function resolveEntitlements(email) {
  ...
  const rows = await dbSelect("users", {
    select: "id,airtable_id,email,name,active,factoryos_role,calculator_role,rate_cards_role,margin_pct,vendor_id,user_clients(client_id,clients(airtable_id))",
