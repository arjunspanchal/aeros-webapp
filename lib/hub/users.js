...
const HR_ROLES = new Set(["admin", "manager"]);
const PAYOUTS_ROLES = new Set(["admin", "accountant"]);
...
export async function resolveEntitlements(email) {
  ...
  const rows = await dbSelect("users", {
    select: "id,airtable_id,email,name,active,factoryos_role,calculator_role,rate_cards_role,hr_role,payouts_role,margin_pct,vendor_id,user_clients(client_id,clients(airtable_id))",
  ...
  const p = normRole(row.payouts_role);
  const orders = r && FACTORYOS_ROLES.has(r) ? r : null;
  const hr = h && HR_ROLES.has(h) ? h : null;
  // Payouts: explicit users.payouts_role wins; otherwise derive — hub admins
  // and factory managers get full payouts access without a separate grant.
  const payouts = (p && PAYOUTS_ROLES.has(p) ? p : null)
    || (orders === "admin" || orders === "factory_manager" ? "admin" : null);
  ...
  // HR and Payouts are standalone entitlements — a user with ONLY hr or
  // ONLY payouts access still resolves.
  if (!calc && !orders && !hr && !payouts) return null;
