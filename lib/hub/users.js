...
export async function resolveEntitlements(email) {
  ...
  const rows = await dbSelect("users", {
    select: "id,airtable_id,email,name,active,factoryos_role,calculator_role,rate_cards_role,margin_pct,user_clients(client_id,clients(airtable_id))",
  ...
  // Reject only if the user has zero entitlements anywhere. Previously
  // this gate only checked calc + orders, so a user whose ONLY access was
  // RFQs (rate_cards_role='client') got "Account not found or inactive"
  // at OTP request time. Including rateCards here lets admin grant RFQ-
  // only access (e.g. arjunspanchal@gmail.com seeing Testing Grounds
  // quotes without any FactoryOS or Calculator role).
  if (!calc && !orders && !rateCards) return null;
