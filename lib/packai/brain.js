...
// Token-AND matching: every query word must appear in the haystack. Used for
// product/food search where the query is a tight term ("12oz double wall cup").
function matchesQuery(haystack, query) {
  return stemTokens(query).every((t) => haystack.includes(t));
}
  ...
  return {
    ...
    lookupAerosFacts: tool({
      ...
      execute: async ({ query }) => {
        ...
        // Rank by token-overlap (OR, not AND): the model often sends a
        // kitchen-sink query, so score each fact by how many query words it
        // contains and return the best — never a positional first-N slice
        // (which would hide newer facts as the bank grows).
        ...
        const out = (hits.length ? hits : scored.slice(0, 6)).map((s) => s.f);
