...
async function getCatalog() {
  if (!_catalog) {
    _catalog = await packaiSelect('v_packai_catalog', {
      select:
        'id,product_name,sku,category,sub_category,size_volume,material,colour,gsm,wall_type,coating,units_per_case,carton_dimensions,top_diameter_mm,moq_units,print_moq_units,lead_time,stock_status,sell_price_inr',
    });
  }
  return _catalog;
}
...
ALWAYS call lookupAerosFacts BEFORE answering any question about Aeros specifically — what we make or don't make, product sizes/ranges, materials, coatings, care/storage, printing, lead times, MOQ, samples, ordering, stock availability, custom/non-standard sizes, moulds/dies and tooling charges, or any Aeros policy or process. Do this even if you think you know the answer, and ALWAYS before deflecting to "contact the team" or saying "I don't have that information" — a verified fact very likely exists. Prefer a verified fact over your own general knowledge and over any generic deflection. Only for generic packaging science not specific to Aeros, answer from general knowledge.
...
- If the customer asks about minimum order / MOQ, use the product's moqPlainUnits (plain stock) and moqPrintedUnits (printed/branded) — printed MOQs are higher. State them per product when relevant.
...
Confidentiality applies ONLY to our INTERNAL supplier/purchase costs, dealer/trade pricing, and vendor/mill identities: if asked about those, say you don't have that information, never name a supplier or vendor, and offer to connect them with the Aeros team. This does NOT apply to customer-facing charges — tooling/die/mould costs, MOQ, product prices and lead times are legitimate to share from a verified fact or the catalog. Never let the confidentiality rule stop you from sharing a customer-facing tooling/die cost that exists as a verified fact.`;
      ...
      execute: async ({ query, category }) => {
        ...
        return {
          ...
          results: results.map((p) => ({
            ...
            moqPlainUnits: p.moq_units || null,
            moqPrintedUnits: p.print_moq_units || null,
