...
// (keylines and mockups). "Keyline" covers everything die-cut/printable
// — the same artefact is variously called KLD, keyline, or outline in the
// industry; we collapse them to one type to avoid pointless taxonomy.
// Reads/writes route through the shared Supabase REST helpers; binary
// objects live in the private `design-files` bucket and are served via
// signed URLs.
...
// "Keyline" covers KLD / keyline / outline (all the same thing — the
// die-cut / printable-area artwork). Mockup = render / 3D / paper-proof.
export const FILE_TYPES = ["Keyline", "Mockup", "Other"];
...
// Plastic lids (PET / PP) are injection-molded and don't have keylines —
// they're not customisable, so they shouldn't pollute the Design module's
// product list. Add to this set if more non-customisable categories
// surface (e.g. clear PET cups, if you decide they're also exclusion-
// worthy — confirm before extending).
const NON_CUSTOMISABLE = (p) => {
  const mat = (p.material || "").toUpperCase();
  const cat = (p.category || "").toLowerCase();
  return cat === "lids" && (mat === "PET" || mat === "PP" || mat === "RPET");
};
...
// don't yet have any files — that's the common upload path. Plastic lids
// are filtered out (see NON_CUSTOMISABLE).
export async function listProductsWithDesignSummary({ search = "" } = {}) {
  ...
  // Map productId → { totalCount, byType: { Keyline: 2, Mockup: 1, ... } }
