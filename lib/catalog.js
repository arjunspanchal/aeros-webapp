...
async function attachPricingTiers(products) {
  ...
  try {
    rows = await dbSelect("master_product_pricing", {
      select:
        "id,offering_type,min_qty,incoterm,price_inr,price_usd,fob_india_inr,fob_india_usd,india_landed_inr,usa_landed_usd,notes,master_products(sku)",
      order: "min_qty.asc",
    });
  } catch {
  ...
  for (const p of products) {
    p.pricingTiers = (p.sku && bySku.get(p.sku)) || [];
    // Back-compat shim: code paths that pre-date the tier table (notably
    // `computeLandedPrices` and the chat / express-ship calculators) read
    // a single `pricePerUnit` value. Derive it from the lowest-MOQ plain
    // tier with a populated INR price so those callers keep working.
    const lowestPlain = p.pricingTiers
      .filter((t) => t.offeringType === "plain" && t.priceInr != null)
      .sort((a, b) => (a.minQty || 0) - (b.minQty || 0))[0];
    p.pricePerUnit = lowestPlain ? lowestPlain.priceInr : null;
  }
  return products;
}
