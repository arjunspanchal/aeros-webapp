...
// Rates are EXW India, INR only. PP (polypropylene, resin code 5) is a tough,
// flexible, translucent/frosted plastic — no board GSM, no kraft finish.
...
const GROUP_BLURB = {
  ...
  "Flat PP Lid":
    "Flat PP lids — thermoformed and injection-molded — sized to the cup's top diameter. Supplied plain.",
  "Sipper PP Lid":
...
export async function fetchPpCupsAndLids() {
  ...
  // lids entirely (it's the "PP Cups & IM Lids" sheet).
  const imLids = lids.filter((l) => formingOf(l.product_name) !== "Thermoformed");
