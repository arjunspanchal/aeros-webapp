...
// custom-branded pricing rows from the `v_product_pricing` view, joins them in JS,
// and shapes a render-ready list grouped by item type (F-Bottom / U-Bottom PP
// Cup, Cup + Lid Set, Dome / Sipper PP Lid).
//
...
// Each slab carries two pricing bases from the rulebook view v_product_pricing:
// EXW Aeros (FCL self-import, ×1.10) and DDP India (app sell — freight, margin
// and GST included). Lids are IM only (TF excluded) and
...
const GROUP_LABEL = {
  "Flat Bottom PP Cup": "F-Bottom Cups",
  "U-Shape PP Cup": "U-Bottom Cups",
...
const GROUP_BLURB = {
  "Flat Bottom PP Cup":
    "F-Bottom polypropylene cold cups — a flat base for a clean, stable stand. Clear or frosted; plain or custom-branded.",
  "U-Shape PP Cup":
    "U-Bottom PP cups — a rounded base that nests tightly for efficient shipping. Clear or frosted; plain or custom-branded.",
  "Cup + Lid Set":
...
const SHORT_CODE = {
  "Flat Bottom PP Cup": "F-Bottom",
  "U-Shape PP Cup": "U-Bottom",
  "Cup + Lid Set": "Sets",
  "Dome PP Lid": "Dome",
...
export async function fetchPpCupsAndLids() {
  ...
  const [cups, lids] = await Promise.all([
    dbSelect("master_products", {
      ...
      // sold_loose=true only: combo-only components (the cup/lid inside a set,
      // sold_loose=false) are internal and must not surface on the public sheet.
      filter: { category: "eq.PP Cups", sold_loose: "is.true" },
      order: "sku.asc",
    }),
    dbSelect("master_products", {
      select:
        "id,sku,product_name,category,sub_category,size_volume,material,colour,units_per_case,country_of_origin,item_weight_g,carton_dimensions",
      filter: { category: "eq.Lids", sub_category: "like.*PP*", sold_loose: "is.true" },
      order: "sku.asc",
    }),
  ]);
  ...
  // Pricing comes from the rulebook view (rate-structure PDF): every figure is
  // derived live from purchase_inr × pricing_config factors, never hand-stored.
  // We select ONLY the two public bases — fcl_self_import_inr (EXW Aeros,
  // ×1.10) and app_sell_inr (DDP India: (base + freight) × margin × GST,
  // GST-inclusive). purchase / landed columns stay internal.
  const pricing = await dbSelect("v_product_pricing", {
    select: "product_id,min_qty,offering_type,fcl_self_import_inr,app_sell_inr",
    filter: {
  ...
  // Collect priced slabs per product, split by offering type. priceInr is the
  // EXW / FCL self-import rate; ddpInr is the delivered App-Sell rate (null
  // where purchase isn't costed yet → shown "on request").
  const plainByProduct = new Map();
  const printedByProduct = new Map();
  for (const p of pricing) {
    if (p.fcl_self_import_inr == null) continue; // uncosted rows = on request
  ...
  for (const item of products) {
    ...
    const row = {
      ...
      // Cup bottom profile, tagged on each item: F-Bottom (flat base) or
      // U-Bottom (rounded base). Sets/lids carry no profile.
      profile: profileOf(item.sub_category),
      // Cup finish for the Clear/Frosted filter: colour column first, name as
      // fallback (Lorven cups have colour null but "Frosted" in the name).
      finish: isLid ? null : finishOf(item.colour, item.product_name),
      ...
      // Shared rim: lids carry their Ø; cups carry their top diameter (TD).
      // Lets the volume filter keep the lids that fit the selected cups.
...
// Cup bottom profile from the sub-category: F-Bottom (flat base) or U-Bottom
// (rounded base). Lids and cup+lid sets return null.
function profileOf(subCategory) {
  if (subCategory === "Flat Bottom PP Cup") return "F-Bottom";
  if (subCategory === "U-Shape PP Cup") return "U-Bottom";
  return null;
}

// Cup finish: Clear or Frosted. The colour column is authoritative; fall back
// to the product name (e.g. Lorven cups have colour null but "Frosted" in the
// name). Unfrosted PP reads as clear/translucent → "Clear".
function finishOf(colour, name) {
  const c = (colour || "").trim().toLowerCase();
  if (c === "frosted") return "Frosted";
  if (c === "clear") return "Clear";
  if (/frosted/i.test(name || "")) return "Frosted";
  return "Clear";
}
...
// Top diameter of a cup: the first dimension after the "|" separator
// ("12oz / 350ml | 90 x 48 x 95 mm (TD x BD x H)" → 90).
