...
// hub cookie today. Reserved for a later phase when `id` lands in the
// cookie payload.
//
// `factoryosUserId` and `factoryosClientIds` were added to the hub cookie
// payload in PR 1.5a so the unified session can serve everything that
// today reaches for the legacy factoryos cookie's `userId` and `clientIds`
// fields. Tokens minted before PR 1.5a don't have these — defaults preserve
// behaviour (null + [] is the same observable state as a session with no
// factoryos data, which is what the legacy fallback would produce too).
export function shape(payload) {
  ...
  return {
    ...
    factoryosUserId: payload.factoryosUserId ?? null,
    factoryosClientIds: Array.isArray(payload.factoryosClientIds)
      ? payload.factoryosClientIds
      : [],
    // Set only for the 'vendor' factoryos role — scopes the vendor portal to
    // a single vendor record. Null for every other role / legacy cookie.
    factoryosVendorId: payload.factoryosVendorId ?? null,
  };
}
