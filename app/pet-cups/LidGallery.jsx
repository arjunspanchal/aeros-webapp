// "Lid styles" gallery for the public PET cups & lids page. Server component
// (no interactivity). One card per PET lid SKU, with a product image loaded
// from public/pet-cups/lids/<SKU>.<ext>. Drop a photo in using the SKU as the
// filename (any of .jpg / .jpeg / .png / .webp) and it appears automatically;
// until then a labelled placeholder renders so the section never shows a
// broken image.

import fs from "node:fs";
import path from "node:path";

const LID_IMG_DIR = path.join(process.cwd(), "public", "pet-cups", "lids");

function findLidImage(sku) {
  for (const ext of ["jpg", "jpeg", "png", "webp"]) {
    try {
      if (fs.existsSync(path.join(LID_IMG_DIR, `${sku}.${ext}`))) {
        return `/pet-cups/lids/${sku}.${ext}`;
      }
    } catch {
  ...
  return (
    <section id="lid-styles" className="mt-12">
      ...
      <p className="mt-2 max-w-2xl text-sm text-ink-600">
        Every clear PET lid in the range — flat, dome and sipper. All supplied plain; match a lid
        to your cup by its top diameter.
      </p>
            ...
            >
              <div className="aspect-square w-full bg-ink-50">
                {src ? (
                  ...
                  <img
                    src={src}
                    alt={`${lid.typeLabel} — ${title} — Aeros PET lid (${lid.sku})`}
