...
export async function PATCH(req, { params }) {
  ...
  // Validate optional PDF replacement payload (skipped when clearPdf=true).
  if (!body.clearPdf && body.fileBase64) {
    if (!body.filename) {
      return Response.json({ error: "Filename is required when uploading a new PDF" }, { status: 400 });
    }
